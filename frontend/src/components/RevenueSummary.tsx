...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', debugTenant, showRaw }) => {
    ...
    // `total_revenue` is provided by backend as a 2-decimal string to avoid float rounding.
    const displayTotal = data.total_revenue;
    const [majorRaw, minorRaw] = displayTotal.split(".");
    const majorFormatted = Number(majorRaw).toLocaleString(undefined);
    const minorFormatted = (minorRaw ?? "00").padEnd(2, "0").slice(0, 2);
                    ...
                    <div>
                        ...
                        <div className="flex items-baseline gap-2 mt-1">
                            <span className="text-3xl font-bold text-gray-900 tracking-tight">
                                {data.currency} {majorFormatted}.{minorFormatted}
                            </span>
